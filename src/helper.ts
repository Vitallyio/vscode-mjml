...
export async function renderMJML(cb: (content: string) => any, fixImg?: boolean, minify?: boolean, beautify?: boolean): Promise<void> {
  ...
  let content = await compileHtml(activeTextEditor.document.getText());
...
function resolveFileOrText(fileName: string): string | undefined {
...
export async function compileHtml(html: string): Promise<string> {
  const activeTextEditor: TextEditor | undefined = window.activeTextEditor;
  if (!activeTextEditor) {
